...
import {
  Trophy, Users, Plus, Crown, ChevronRight,
  Clock, Copy, Check, Link2, Flame, Target, Hash,
} from "lucide-react";
...
const Challenges = () => {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setForm({
        ...
        target_value: 1, unit: "times", category: "fitness",
      });
  ...
  const handleJoinByCode = async (e) => {
    ...
    setJoiningCode(true);
    try {
      const { data } = await api.post("/challenges/join-by-code", { inviteCode });
  ...
  const handleLog = async (id, e) => {
    e.stopPropagation();
    setLoggingId(id);
    try {
      const { data } = await api.post(`/challenges/${id}/log`);
      ...
      setChallenges((prev) =>
        prev.map((c) =>
          c.id === id
            ? { ...c, today_value: data.value, today_completed: data.completed }
            : c
        )
      );

      // Refresh leaderboard if viewing this challenge
      if (selected === id) fetchLeaderboard(id);
    } catch (err) {
  ...
  return (
    <PageWrapper>
      <div className="min-h-screen pt-16 pb-24 sm:pb-10"
              ...
              {loading ? (
                <div className="flex flex-col gap-3">
                  {[1, 2].map((i) => (
                    <div key={i} className="card p-5 h-36 animate-pulse">
                      <div className="h-5 rounded-lg w-3/4 mb-3"
                        style={{ background: "var(--bg-muted)" }} />
                      <div className="h-3 rounded-lg w-1/2 mb-4"
                        style={{ background: "var(--bg-muted)" }} />
                ...
                <div className="card p-10 text-center">
                  ...
                  <div className="flex gap-2 justify-center">
                    ...
                    <Button variant="ghost" onClick={() => setShowJoinCode(true)}
                      className="gap-2" size="sm">
                      <Link2 size={13} /> Join
                    </Button>
                  </div>
                ...
                <div className="flex flex-col gap-3">
                  {challenges.map((c, i) => {
                    const cfg      = CATEGORY_CONFIG[c.category] || CATEGORY_CONFIG.fitness;
                    const target   = c.target_value || 1;
                    const current  = c.today_value  || 0;
                    const progress = Math.min((current / target) * 100, 100);
                    const done     = c.today_completed || false;
                    ...
                    return (
                      <motion.div
                        ...
                        {/* Today's progress + log button */}
                        <div
                          className="mt-3 p-3 rounded-xl"
                          style={{
                            background: done ? "rgba(34,197,94,0.06)" : "var(--bg-muted)",
                            border:     `1px solid ${done ? "rgba(34,197,94,0.2)" : "var(--border-muted)"}`,
                          }}>
                          <div className="flex items-center justify-between mb-2">
                            <span
                              ...
                              {done ? "✅ Done today!" : "Today's Progress"}
                            </span>
                            <span
                              className="text-xs tabular-nums font-bold"
                              style={{ color: done ? "#22c55e" : "var(--text-faint)" }}>
                              {current}/{target} {c.unit}
                            </span>
                          </div>
                          ...
                          <div
                            ...
                            <motion.div
                              ...
                              style={{ background: done ? "#22c55e" : "#f97316" }}
                              initial={{ width: 0 }}
                              animate={{ width: `${progress}%` }}
                              transition={{ duration: 0.5 }} />
                          </div>

                          {/* Leaderboard + Log buttons */}
                          <div className="flex gap-2">
                            ...
                            <motion.button
                              onClick={(e) => handleLog(c.id, e)}
                              disabled={loggingId === c.id}
                              className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold"
                              style={{
                                background: done ? "rgba(34,197,94,0.1)" : "#22c55e",
                                color:      done ? "#22c55e"             : "#052e16",
                                border:     done ? "1px solid rgba(34,197,94,0.3)" : "none",
                                fontFamily: "var(--font-display)",
                                opacity:    loggingId === c.id ? 0.7 : 1,
                              }}
                              whileTap={{ scale: 0.95 }}>
                              {loggingId === c.id ? (
              ...
              <div className="card p-6 min-h-[320px] flex flex-col">
                ...
                {selectedChallenge && (
                  <div
                    ...
                    <span className="flex items-center gap-1.5 text-xs font-semibold"
                      style={{ color: "var(--text-muted)" }}>
                      <Clock size={12} /> {selectedChallenge.duration_days} days
                    </span>
                    <span className="flex items-center gap-1.5 text-xs font-semibold"
                      style={{ color: "var(--text-muted)" }}>
                      <Target size={12} />
                      {selectedChallenge.target_value} {selectedChallenge.unit}/day
                    </span>
                    <span className="flex items-center gap-1.5 text-xs font-semibold"
                      style={{ color: "var(--text-muted)" }}>
                      <Users size={12} /> {selectedChallenge.participant_count} participants
                    </span>
                    <span className="flex items-center gap-1.5 text-xs font-semibold"
                      style={{ color: "var(--text-muted)" }}>
                      <Flame size={12} />
                      {CATEGORY_CONFIG[selectedChallenge.category]?.label || selectedChallenge.category}
                    </span>
                  </div>
                )}
                ...
                {!selected ? (
                  ...
                  <div
                    ...
                    style={{ color: "var(--text-faint)" }}>
                    <Users size={36} />
                    <p className="text-sm font-syne font-bold">No participants yet</p>
                      ...
                      {leaderboard.map((entry, i) => (
                        <motion.div
                          ...
                          <div
                            ...
                            style={{
                              background: i < 3 ? `${medalColors[i]}18` : "var(--bg-card)",
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <div
                                ...
                                style={{ background: "var(--border)", maxWidth: "100px" }}>
                                <motion.div
                                  className="h-full rounded-full"
                                  style={{ background: i < 3 ? medalColors[i] : "#22c55e" }}
                                  initial={{ width: 0 }}
                                  animate={{ width: `${Math.min(entry.completionRate, 100)}%` }}
                          ...
                          <div className="text-right shrink-0">
                            <p
                              ...
                              style={{ color: i < 3 ? medalColors[i] : "var(--text)" }}>
                              {entry.score}
                            </p>
          ...
          <form onSubmit={handleCreate} className="flex flex-col gap-4">
            ...
            <div className="flex gap-3 mt-1">
              <Button variant="ghost" type="button"
                onClick={() => setShowCreate(false)} className="flex-1">
                Cancel
              </Button>
              <Button type="submit" loading={creating} className="flex-1 gap-1.5">
        ...
        {/* ─── Join by Code Modal ───────────────────── */}
        <Modal open={showJoinCode} onClose={() => setShowJoinCode(false)}
          ...
          <form onSubmit={handleJoinByCode} className="flex flex-col gap-4">
            ...
            <div className="flex gap-3">
              <Button variant="ghost" type="button"
                onClick={() => setShowJoinCode(false)} className="flex-1">
                Cancel
              </Button>
              <Button type="submit" loading={joiningCode} className="flex-1 gap-1.5">
